...
- **修复主窗口无法记住上次调整大小的问题（#278）。** 启动阶段在窗口尚未归属具体显示器时会回退到主显示器应用已保存的逻辑尺寸，不再因 `current_monitor` 暂时为空而跳过恢复。用户拖动调整窗口时会实时记录最后一个有效的非最大化尺寸，退出时统一写入配置，避免关闭、最小化或安装更新过程中的瞬时原生尺寸覆盖用户选择；恢复尺寸仍会按当前显示器可用范围约束。
...
- **Fix the main window forgetting its last adjusted size (#278).** Startup now falls back to the primary monitor when the not-yet-visible window has no current monitor, so the saved logical dimensions are actually restored. Resize events retain the latest valid non-maximized size in memory and flush it with the rest of the layout on exit, preventing transient close, minimize, or installer geometry from replacing the user's choice. Restored dimensions remain constrained to the current monitor's available area.
